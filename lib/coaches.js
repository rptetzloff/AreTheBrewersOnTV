...
import { allGames, allRawGames } from './seasons.js';
...
for (const [gid, mgr] of parseGameinfoMgr(allRawGames)) {
	if (!gidToMgr.has(gid)) gidToMgr.set(gid, mgr);
}
const mgrNames = parseBiofile(readFileSync(BIOFILE, 'utf8'));
export const coaches = computeCoachesFromData(allGames, gidToMgr, mgrNames, champions);

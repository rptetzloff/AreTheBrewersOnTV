  ...
  switch (slug) {
    ...
    case 'world-series-appearances': {
      ...
      return recordsFrame({
        ...
        big: String(data.worldSeriesAppearances.length),
        context: `seasons: ${data.worldSeriesAppearances.map((p) => p.season).join(', ')}`,
        runners: rest.length ? [`also: ${rest.map((p) => p.season).join(', ')}`] : ['The only one in franchise history.'],
      });
    }
    case 'playoff-appearances': {
      ...
      return recordsFrame({
        ...
        context: `seasons: ${data.playoffAppearances.map((p) => p.season).join(', ')}`,
        runners: rest.length ? [`also: ${rest.map((p) => p.season).join(', ')}`] : ['The only one in franchise history.'],
      });
    }
    case 'win-streaks': {
    ...
    default: {
      ...
      const rows = [
        ...
        `World Series appearances — ${ws.length ? ws.map((p) => p.season).join(', ') : 'none'}`,

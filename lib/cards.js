...
// Big centered word. The full text (trailing punctuation included) centers
// on x=600 — under a single centered baseball, letters-only centering reads
// as the word being shoved sideways. `gap` px separate the baseball's
// bottom edge from the letter cap-tops.
...
export function buildRecordsSvg(slug, data) {
  ...
  switch (slug) {
    ...
    case 'world-series-appearances': {
      const [top, ...rest] = data.championshipAppearances;
      if (!top) {
    ...
    case 'losing-streaks': {
      ...
      if (!top) {
        return frame(title('LONGEST LOSING STREAKS') + sub(168, `Milwaukee Brewers · ${range}`)
          + line(360, 'None. Somehow.', WHITE, 60));
      }
      return recordsFrame({
        heading: 'LONGEST LOSING STREAKS', range,
        big: `${top.games} straight`, bigColor: WHITE,
        context: `${formatDate(top.startDate)} – ${formatDate(top.endDate)}`,
        runners: [rest.map((s) => `${s.games} in ${streakSpan(s)}`).join('  ·  ')],
      });
    }
    case 'player-error-game': {
      const [top, ...rest] = data.playerErrorGames || [];
      if (!top) {
        return frame(title('MOST ERRORS IN A GAME · PLAYER') + sub(168, `Milwaukee Brewers · ${range}`)
          + line(360, 'No data', WHITE, 60));
      }
      return recordsFrame({
        heading: 'MOST ERRORS IN A GAME · PLAYER', range,
        big: `${top.e} E`, bigColor: WHITE,
        context: `${top.player} vs the ${top.opponent} · ${formatDate(top.date)}`,
        runners: [rest.slice(0, 3).map((g) => `${g.e} — ${g.player} (${g.season})`).join('  ·  ')].filter(Boolean),
      });
    }
    case 'team-error-game': {
      const [top, ...rest] = data.mostTeamErrorGames || [];
      if (!top) {
        return frame(title('MOST ERRORS IN A GAME · TEAM') + sub(168, `Milwaukee Brewers · ${range}`)
          + line(360, 'No data', WHITE, 60));
      }
      return recordsFrame({
        heading: 'MOST ERRORS IN A GAME · TEAM', range,
        big: `${top.e} E`, bigColor: WHITE,
        context: `vs the ${top.opponent} · ${formatDate(top.date)}`,
        runners: [rest.slice(0, 3).map((g) => `${g.e} vs ${g.opponent} (${g.season})`).join('  ·  ')].filter(Boolean),
      });
    }
    case 'worst-starts': {
    ...
    case 'worst-losses': {
      ...
      const flag = (g) => (g.championship ? ', World Series' : g.playoff ? ', playoffs' : '');
      return recordsFrame({
        heading: win ? 'MOST LOPSIDED WINS' : 'WORST LOSSES', range,
        big: `${top.pf}–${top.pa}`, bigColor: win ? GOLD : WHITE,
        context: `vs the ${top.opponent} · ${formatDate(top.date)}${top.championship ? ' · World Series' : top.playoff ? ' · Playoffs' : ''}`,
    ...
    case 'player-rbi-game': {
      ...
      const [top, ...rest] = h;
      if (!top) {
        return frame(title('MOST RBIS IN A GAME · PLAYER') + sub(168, `Milwaukee Brewers · ${range}`)
          + line(360, 'No data', WHITE, 60));
      }
      return recordsFrame({
        heading: 'MOST RBIS IN A GAME · PLAYER', range,
        big: `${top.rbi} RBI`,
        context: `${top.player} vs the ${top.opponent} · ${formatDate(top.date)}`,
        runners: [rest.slice(0, 3).map((g) => `${g.rbi} — ${g.player} (${g.season})`).join('  ·  ')].filter(Boolean),
      });
    }
    case 'cycles': {
      const c = data.cycles || [];
    ...
    default: {
      ...
      const ws = data.championshipAppearances, po = data.playoffAppearances;
      const rows = [

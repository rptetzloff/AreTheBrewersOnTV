...
import { parseGameinfoCsv, parseGamesCsv } from '../records-core.js';
...
const gameinfoCsvText = readFileSync(GAMEINFO, 'utf8');
const allRawGames = parseGamesCsv(gameinfoCsvText);
const allGames = parseGameinfoCsv(
  gameinfoCsvText,
  readFileSync(NAMES, 'utf8'),
  readFileSync(TEAMSTATS, 'utf8'),
);
...
function stateFromCounts({ season, wins, losses, ties, worldSeriesName, isPast }) {
  if (worldSeriesName) return { kind: 'champions', season, record: rec(wins, losses, ties), worldSeriesName };
...
function csvState(season) {
  ...
  let w = 0, l = 0, t = 0; let worldSeriesName = null;
  for (const g of games) {
    const result = g['Brewers Win'];
    if (g.regular_season === '1') {
      if (result === 'WIN') w++; else if (result === 'LOSS') l++; else if (result === 'TIE') t++;
    }
    if (g.worldseries && g.worldseries.trim() !== '' && result === 'WIN') {
      worldSeriesName = `World Series ${g.worldseries.toUpperCase()}`;
    }
  }
  return stateFromCounts({ season, wins: w, losses: l, ties: t, worldSeriesName, isPast: true });
}
...
async function espnState(season) {
  ...
  if (!events.length || isOffseason(events)) {
    ...
    state = stateFromCounts({ season: year, wins: w, losses: l, ties: t, worldSeriesName, isPast: false });
  }
...
export { maxSeason, allGames, allRawGames };
